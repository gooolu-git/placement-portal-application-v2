<script setup>
import { ref, onMounted } from 'vue';
import axios from 'axios';

const selectedStudents = ref([]);
const companyDetails = ref({ company_name: '' });

// Fetch selected candidates
const fetchSelected = async () => {
  try {
    const token = localStorage.getItem('token');
    // Note: Ensure your backend has an endpoint that returns the company details 
    // and the list of applications where status == 'Selected'
    const res = await axios.get('http://localhost:5000/company/selected-candidates', {
      headers: { Authorization: `Bearer ${token}` }
    });
    selectedStudents.value = res.data.data;
    companyDetails.value = res.data.company;
  } catch (err) {
    console.error("Error fetching selected:", err);
  }
};

const formatDate = (dateStr) => {
  return new Date(dateStr).toLocaleDateString('en-GB', { month: 'short', day: '2-digit', year: 'numeric' });
};

onMounted(fetchSelected);
</script>

<template>
  <div class="container py-4 mt-5 bg-light ">
    <!-- Header -->
    <div class="d-md-flex justify-content-between align-items-center mb-4">
      <div class="border-start border-primary border-4 ps-3">
        <h2 class="fw-bold text-dark mb-1">Selected <span class="text-primary">Candidates</span></h2>
        <p class="text-secondary mb-0 small text-uppercase fw-bold" style="letter-spacing: 1px;">
          Hired talent for <span class="text-dark">{{ companyDetails.company_name }}</span>
        </p>
      </div>
      <div class="mt-3 mt-md-0 d-flex gap-2">
        <button onclick="window.print()" class="btn btn-white border rounded-pill fw-bold px-4 shadow-sm btn-sm">
          <i class="fas fa-print me-2 text-primary"></i>Export Report
        </button>
        <router-link to="/company/dashboard" class="btn btn-primary rounded-pill fw-bold px-4 shadow-sm btn-sm">
          Dashboard
        </router-link>
      </div>
    </div>

    <!-- Stats Card -->
    <div class="row mb-4">
      <div class="col-md-3">
        <div class="card border-0 shadow-sm rounded-4 p-3 bg-white">
          <div class="d-flex align-items-center gap-3">
            <div class="bg-success bg-opacity-10 text-success rounded-circle d-flex align-items-center justify-content-center" style="width: 45px; height: 45px;">
              <i class="fas fa-user-check"></i>
            </div>
            <div>
              <div class="h5 fw-bold mb-0 text-dark">{{ selectedStudents.length }}</div>
              <div class="text-muted small fw-bold text-uppercase" style="font-size: 0.6rem;">Total Hires</div>
            </div>
          </div>
        </div>
      </div>
    </div>

    <!-- Table -->
    <div class="card border-0 shadow-sm rounded-4 overflow-hidden bg-white">
      <div class="table-responsive">
        <table class="table align-middle mb-0">
          <thead class="table-light">
            <tr class="text-uppercase text-secondary" style="font-size: 0.7rem; letter-spacing: 0.05em;">
              <th class="ps-4 py-3 border-0">Candidate Name</th>
              <th class="py-3 border-0">Job Role</th>
              <th class="py-3 border-0">Academic Info</th>
              <th class="py-3 border-0">Status</th>
              <th class="text-end pe-4 py-3 border-0">Resume</th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="app in selectedStudents" :key="app.id" class="border-bottom">
              <td class="ps-4 py-3">
                <div class="d-flex align-items-center">
                  <div class="position-relative me-3">
                    <img :src="`https://api.dicebear.com/7.x/avataaars/svg?seed=${app.student_applicant.user.name}`" 
                         alt="Avatar" class="rounded-circle border border-2 p-1 bg-light" width="45" height="45">
                    <span class="position-absolute bottom-0 end-0 bg-success border border-white border-2 rounded-circle p-1"></span>
                  </div>
                  <div>
                    <div class="fw-bold text-dark" style="font-size: 0.9rem;">{{ app.student_applicant.user.name }}</div>
                    <div class="text-muted" style="font-size: 0.7rem;">ID: #{{ app.student_applicant.user.id }}</div>
                  </div>
                </div>
              </td>
              <td>
                <div class="fw-bold text-dark" style="font-size: 0.85rem;">{{ app.target_drive.job_title }}</div>
                <div class="text-muted small" style="font-size: 0.7rem;">Hired: {{ formatDate(app.applied_on) }}</div>
              </td>
              <td>
                <div class="fw-bold text-dark small">{{ app.student_applicant.department }}</div>
                <div class="badge bg-info-subtle text-info border border-info-subtle rounded-pill" style="font-size: 0.65rem;">
                  CGPA: {{ app.student_applicant.cgpa }}
                </div>
              </td>
              <td>
                <span class="fw-bold px-3 py-1 rounded-pill border small bg-success-subtle text-success border-success-subtle" style="font-size: 0.65rem;">
                  <i class="fas fa-check me-1"></i> SELECTED
                </span>
              </td>
              <td class="text-end pe-4">
                <a v-if="app.student_applicant?.resume" 
                   :href="`http://localhost:5000/static/uploads/${app.student_applicant.resume}`" 
                   target="_blank" class="btn btn-outline-primary btn-sm rounded-pill px-3 fw-bold">
                  <i class="fas fa-file-pdf me-1"></i> View
                </a>
                <span v-else class="badge bg-light text-muted fw-normal border">N/A</span>
              </td>
            </tr>
            <tr v-if="selectedStudents.length === 0">
              <td colspan="5" class="text-center py-5">
                <div class="text-secondary opacity-25 mb-3">
                  <i class="fas fa-user-slash fa-4x"></i>
                </div>
                <h5 class="text-muted fw-bold">No selections recorded</h5>
              </td>
            </tr>
          </tbody>
        </table>
      </div>
    </div>
  </div>
</template>